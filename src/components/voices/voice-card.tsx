...
const paperStyles = [
  { rotation: "-0.8deg", tone: "bg-[#e6dfd1]", size: "p-7 sm:p-9", folded: true },
  { rotation: "0.5deg", tone: "bg-[#f0eee7]", size: "p-6 sm:p-7", folded: false },
  { rotation: "-0.35deg", tone: "bg-[#d9cbb1]", size: "p-8 sm:p-11", folded: false },
  { rotation: "0.75deg", tone: "bg-[#e9e3d7]", size: "p-6 sm:p-8", folded: true },
  { rotation: "-0.55deg", tone: "bg-[#ddd6c7]", size: "p-7 sm:p-10", folded: false },
  { rotation: "0.2deg", tone: "bg-[#f2f0e9]", size: "p-6 sm:p-7", folded: false },
];
...
export function VoiceCard({ voice, index }: { voice: Voz; index: number }) {
  const [count, setCount] = useState(voice.yo_tambien);
  ...
  async function respond() {
    ...
    const response = await fetch(`/api/voces/${voice.id}/yo-tambien`, { method: "POST" });
  ...
  return (
    <RevealOnView delay={(index % 3) * 0.08}>
      <article
        className={`paper-grain relative mb-7 break-inside-avoid border border-black/10 text-ink shadow-[0_20px_50px_rgb(0_0_0_/_0.24)] transition-transform duration-700 hover:translate-y-[-2px] ${paper.tone} ${paper.size} ${paper.folded ? "paper-fold" : ""}`}
        style={{ transform: `rotate(${paper.rotation})` }}
      >
      ...
      <div className="mt-10 flex items-end justify-between gap-4 border-t border-ink/10 pt-4">
        ...
        <div className="flex items-center gap-3">
          <button
            className="inline-flex min-h-9 items-center gap-1.5 text-xs text-ink/65 transition-colors hover:text-ink focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ink disabled:cursor-default"
            onClick={respond}
            disabled={hasResponded}
            aria-label="Yo también"
          >
            <Heart className={`size-3.5 ${hasResponded ? "fill-current" : ""}`} aria-hidden="true" />
            {count > 0 ? count : "Yo también"}
          </button>
          <button
